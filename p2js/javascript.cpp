...
void JavaScriptVM::ReportException(v8::TryCatch* tryCatch) {
	...
	if(message.IsEmpty()) {
		...
		v8::Local<v8::Context> context = this->globalContext.Get(this->isolate);
...
void JavaScriptVM::ReleaseScript(HSCRIPT hScript) {
	if(hScript) {
		auto globalScript = reinterpret_cast<v8::Global<v8::Script>*>(hScript);
		delete globalScript;
	}
}
...
ScriptStatus_t JavaScriptVM::ExecuteFunction(HSCRIPT hFunction, ScriptVariant_t* pArgs, int nArgs, ScriptVariant_t* pReturn, HSCRIPT hScope, bool bWait) {
	...
	//v8::Global<v8::Function>* globalScript = reinterpret_cast<v8::Global<v8::Function>*>(hFunction);
	//v8::Local<v8::Function> script = globalScript->Get(this->isolate);
	//script->Call(context, context->Global(), 0, nullptr);
